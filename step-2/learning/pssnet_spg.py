...
def pssnet_spg_edge_features(edges, node_att, edge_att, args):
    ...
    fea_bank = ['linearity', 'verticality', 'curvature', 'sphericity', 'planarity', \
                'xyz', 'vcount', 'triangle_density', 'inmatrad', \
                'shape_descriptor', 'compactness', 'shape_index', 'pt2plane_dist_mean', \
...
def pssnet_spg_reader(args, fname, incl_dir_in_name=False):
    ...
    edge_feats = pssnet_spg_edge_features(edges, node_att, edge_att, args)
...
def load_superpoint(args, fname, id, train, test_seed_offset, s_label):
    ...
    if args.pc_attribs != '':
        ...
        if 'all' in args.pc_attribs:#48
            additional_feas = np.concatenate((additional_feas, np.float32(P[0, 6:53])), axis=0) #points have same feature value within each segment
